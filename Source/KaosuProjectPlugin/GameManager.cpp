...
void MyGameManager::OnHandleCallback(IVisCallbackDataObject_cl *pData)
{
  ...
  if (pData->m_pSender==&Vision::Callbacks.OnUpdateSceneBegin)
  {
      ...
      if (m_bPlayingTheGame)
      {
		...
		for(int i = 0; i < (int)kMap::m_full_path.GetValidSize(); i++)
		{
			VString s;
			s.FormatEx("Path: %d", i);
			Vision::Message.DrawMessage3D(s, kMap::m_full_path[i]);
		}
      }
      return;
  }
...
void MyGameManager::SetPlayTheGame(bool bStatus)
{
  ...
  if (m_bPlayingTheGame)
  {
	 ...
	 kMap* mapGen = (kMap *) Vision::Game.SearchEntity("MapGenerator");
	 VASSERT(mapGen);	
	 mapGen->ClearMap(); // The map is cleared every time in case something wierd happens
	 mapGen->InitMap();
	 MyGameManager::PlacePlayer(mapGen->m_friendlySpawn);
  } 
  else
  {
...
const bool MyGameManager::PlacePlayer(const hkvVec3 position)
{
	VSmartPtr<kCharacter> player = (kCharacter *) Vision::Game.SearchEntity("Player");
	VASSERT(player);
	player->MoveTo(position * 100.7f + hkvVec3(0.f,0.f,90.f));
	return true;
}
